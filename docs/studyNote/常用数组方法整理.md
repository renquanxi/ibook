...
如果是数组对象通过比较其中一个值来排序，也是可以利用上面方法的

```js
// propertyName是需要比较的对象中的属性
// isSequence是否是正序排列 传Boolean
function compare(propertyName, isSequence) {
  let num = isSequence ? -1 : 1
...
let arr = [{x: 2}, {x: 1}, {x: 4}, {x: 9}, {x: 8}]
console.log(arr.sort(compare('x', true)))  // [{x: 1}, {x: 2}, {x: 4}, {x: 8}, {x: 9}]
console.log(arr.sort(compare('x', false))) // [{x: 9}, {x: 8}, {x: 4}, {x: 2}, {x: 1}]
```
